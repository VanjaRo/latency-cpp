...
В рамках соревнования вам нужно написать программу, которая получает данные по ордербукам по многим инструментам и поддерживает корректные ордербуки по заданному заранее подмножеству инструментов. В ордербуке необходимо корректно поддерживать первые 15 ненулевых уровней. Обозначим уровни цен для асков как $p_1^a, p_2^a, ..., p_{15}^a$ и аналогично для бидов $p_i^b$. Обозначим также объемы, на соответствующих уровнях как $q_1^a, ..., q_{15}^a$. Вам необходимо подсчитывать VWAP (volume-weighted average price), считаемый как
```math
VWAP = \frac{\sum_{i=0}^{15} p_i^a \cdot q_i^a + p_i^b \cdot q_i^b}{\sum_{i=0}^{15} q_i^a + q_i^b} 
```
...
2. Непосредственно в данных будет дана информация только про первые 5 ненулевых уровня. Однако в процессе применения апдейта размер массива, описанного выше, может быть больше. Также в рамках задания для подсчета VWAP вам
нужно помнить первые 15 уровней.
...
Для синхронизации снепшотов и апдейтов используюся поля `change_no` в снепшоте и `change_no` в апдейтах. При применении апдейтов если очередной `change_no` не равен предыдущему+1, то необходимо пропустить все апдейты до следующего снепшота (фактически, часть данных была потеряна).
...
Список `instrument1,...` это список интересующих нас инструментов, по которым нужно поддерживать ордербуки.
